...
class MultimodalSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.exists(self.data_dir):
            self.files = sorted(glob.glob(os.path.join(self.data_dir, "*.npz")) + glob.glob(os.path.join(self.data_dir, "*.pt")))

        self.synthetic_backup = None
        if len(self.files) == 0:
            if fallback_to_synthetic:
                print(f"[Dataset] Notice: No sequence files found in {self.data_dir}. Initializing synthetic dataset ({synthetic_samples} samples) for split '{split}'.")
